...

{
      ...
      float depth = texture2D(u_texture2, v_texCoords).x;

      float z = (-1.0) * u_far * u_near / (depth * (u_far + (-1.0) * u_near) + (-1.0) * u_far);

      float k = clamp(abs(z + (-1.0) * u_clarity) / u_fade, 0.0, 1.0);
...

{
	  ...
	  vec2 uv2 = v_texCoords + vec2(u_step.x,0.0);

      vec2 uv3 = v_texCoords + vec2(u_step.z, u_step.w);

      vec2 uv4 = v_texCoords + vec2(0.0, u_step.y);

      vec2 uv5 = v_texCoords + vec2(-u_step.z, u_step.w);

      vec2 uv6 = v_texCoords + vec2(-u_step.x, 0.0);

      vec2 uv7 = v_texCoords + vec2(-u_step.z, -u_step.w);

      vec2 uv8 = v_texCoords + vec2(0.0, -u_step.y);

      vec2 uv9 = v_texCoords + vec2(u_step.z, -u_step.w);
      ...
      float d1 = CalculateMixFactor(v_texCoords);
      ...
      vec4 colorOriginal = texture2D(u_texture0, v_texCoords);
